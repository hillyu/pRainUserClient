...
public class ScanningActivity extends ListActivity {
    ...
    private void startPmonActivity() {

        final Intent intent = new Intent(this, PmonActivity.class);
    ...
    private void startPmonService() {
        ...
        for (int j = 0; j < mDevicesListAdapter.getCount(); j++) {
            if (mDevicesListAdapter.getDevice(j).isSelected() && mDevicesListAdapter != null) {
                tmpDeviceAddrs.add(mDevicesListAdapter.getDevice(j).getAddress());
                tmpDeviceNames.add(mDevicesListAdapter.getDevice(j).getName());
            }
        }
        if (!tmpDeviceAddrs.isEmpty()) {
            ...
            builder1.setMessage("Please select at least one device!");//Fixme: add to string res.
...
//        final BTDeviceListItem device = mDevicesListAdapter.getDevice(position);
...
//
//        startActivity(intent);
        //Fixme: this no longer gets called after adding the checkbox.
    }
    ...
    private void handleFoundDevice(final BluetoothDevice device,
        ...
        final BTDeviceListItem mWrapperDevice = new BTDeviceListItem(device, false);
        // adding to the UI have to happen in UI thread
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mDevicesListAdapter.addDevice(mWrapperDevice, rssi, scanRecord);
